...
int player_location(char (*map)[x_map], t_player *p)
{
    int 	x;
	int 	y;
	int 	i;
...
int c_b_2d_array(char **map, int x, int y)
{
    ...
    // End recursion if the current spot is a land or if this spot has already been visited
