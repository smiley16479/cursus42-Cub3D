...
/*   Updated: 2020/07/06 16:42:49 by adtheus          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**	// deprecated function qui n'affiche que des couleurs unies pour les murs
...
void				display_with_mlx(double x_rad_to_add, double x_rad)
{
	int	x;
...
int					render_next_frame(void)
{
	...
	if (g_su->save == 2)
		...
		display_with_mlx(x_rad_to_add, x_rad);
	if (g_su->save == 1 && !(g_su->save = 0))
		bitmap(0);
	mlx_put_image_to_window(g_su->mlx, g_su->mlx_win,
							g_su->su_img->img_ptr, 0, 0);
	player_mov_hook(g_su->p);
